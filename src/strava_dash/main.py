...
from utils import columns_short, columns_shorter
from utils import (
    get_engine,
    fetch_data,
    convert_units,
...
generate_folium_map(
    activities,
    "heatmaps/heatmap_all_activities.html",
    "Activity type",
    5,
...
def render_content(tab):
    if tab == "activities":
        ...
        return html.Iframe(
            srcDoc=open(
                "heatmaps/heatmap_all_activities.html", "r"
            ).read(),  # Load the saved HTML file
            width="100%",  # Width of the map
